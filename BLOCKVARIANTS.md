...
| File | Type | Owns |
|---|---|---|
| `<Name>BlockData.cs` *(or plain `BlockData`)* | `BlockData` / subclass | stats (mass, friction, flags, tint) + lifecycle hooks (`OnApplied`, `OnLocked`) |
...
and Stubborn are also *clarity* gaps: their control changes have no on-block cue today).
...
| a `LateUpdate` | the motion, using `Cells` / `BaseScales` (scale) and `SetCellsFloat` (animated shader props) |
