...
| **Vortex** | 1 | **inverts** left/right steering | the whole brick is **churning dusk marble** (fixed look, replaces chapter art — the inset-gem overlay was rejected as ugly, Nick July 2026): deep plum body, magenta-rose spiral arms winding into a shallow dark eye, blossom-cream vein cores, lantern-red ember specks orbiting the churn — palette echoes Fangkuai District (its home chapter); standard rounded-box bevel + outline; churns and periodically **reverses** direction (the on-block cue for the flip) | ✅ | `VortexBlockData`, `VortexBlockSkin`, `Vortex.shader` |
...
| **Curse** | 1 | **bury-me countdown** — while ANY top cell is exposed to the sky, every COUNTED placement (`GameEvents.BlockPlaced` - never raw locks: magma pips and off-board losses must not tick, review 2026-08-02) burns one sigil (default 4, `buryWithinPlacements` on the asset — Nick 2026-08-02, bracket in playtesting); at zero it fires: **one life** through the hazard path (`LoseLifeToHazard`, so immunity/Ward/Purifier apply) and it re-arms. Covering every top cell pacifies it — cover = any LANDED block or static terrain (own upper cells included; a still-falling piece never counts), and the burying placement itself never ticks (exposure is re-scanned before the tick). Re-exposing (cover destroyed / knocked off) restarts a FRESH countdown, never a half-burned one - and the placement that knocked the cover off doesn't also burn sigil one. Multiple exposed curses tick independently (N at zero = N lives). Inert once the run is over, below the cull line, or while a rescue (Rebound) is carrying it away. The deliberate INVERSE of the Maw: the Maw punishes building ON it, the Curse punishes NOT doing so. `isHazard: true` | near-black **cursed obsidian** with a deep green cast (fixed look; v3 — Nick killed v1's Maw-adjacent violet AND v2's bone-gray + rune dots + rim glow, 2026-08-02: "the eye IS the countdown"): **an eye on EVERY cell** (Nick 2026-08-02 — the one-eye-per-piece anchor read as a lone odd face; each covered cell's eye shuts individually via its `_Expose`, so half-burying a wide curse closes half its eyes) — closed lid seams while falling (barely differs from dark stone, deliberate), opening in LINEAR steps as sigils burn: sliver → half-lidded cat-eye → on the LAST sigil a huge round bloodshot stare (radial red streaks, contracted pupil twitching with the heartbeat, wet catchlight), with occasional slow blinks while calm; the whole-brick alarm is hairline **Voronoi crack seams** that glow acid green from within as doom rises (blazing + pulsing on the last sigil) — no rims, no dots; gentle uniform tremble grows with doom (whole piece, one shared jitter — the per-cell version left a T-brick's self-covered cell still, Nick's catch); **soul-smoke streams up from exposed top cells** (thicker + faster as doom rises) — the bury-me beacon; firing = a radial DETONATION (core flash falling off outward, two shock rings expanding past the brick edge, cell scale-punch, camera kick); bespoke ElevenLabs voice — `curse_fire`/`curse_tick`/`curse_seal` (SOUNDS.md); buried = eyes shut, cracks dark, smoke out | ✅ | `CurseBlockData`, `CurseBlockSkin`, `CurseBlockBehaviour`, `Curse.shader` |
...
- **Fixed-look bricks refuse foreign cosmetic creep** (`BlockVariantSkin.BlocksForeignOverlays`
  = replace-mode skins; Nick 2026-08-02): a vine growing over the Curse's eye or the Bomb's
  fuse hides exactly the signal the brick exists to show. Physical effects (welds, joints)
  still apply — only the LOOK is refused. Anything new that spreads visuals onto neighbouring
