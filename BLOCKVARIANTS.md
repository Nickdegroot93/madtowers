...
| **Pyramid** | 1 | **temporary support, no flat top** — a shape-bound 3-column monument. The original three base cells and sloped `LandableSlope` polygon remain unchanged, including the off-centre apex. After its own landing, the next **three counted placements** charge its engine. A 0.45 s scaled ignition then removes it unconditionally, even under other bricks: standing count −1, no departure life charge; unsupported neighbours fall through ordinary physics. Historical score/placement totals remain intact. `canRotate:false`, `lockDefaultData:true`; spawn frequency unchanged. See §8. | fixed Egyptian sandstone: warm worn courses, carved cracks, stone plinth and three inset sun seals that kindle amber. Small recessed underside exhaust appears only at ignition. No metal-window chassis, teal trim or coloured outline. | ✅ | `PyramidBlockData`, `PyramidBlockBehaviour`, `PyramidBlockSkin`, `PyramidDepartureFx`, `CyberPyramid.shader`; original prefab/definition |
...
## 8. Departing Pyramid (September 2026)

- `GameEvents.BlockPlaced` is the only charge source. The ledger's `LastPlacedBlock`
  excludes its own landing, including when multiple placements happen in one frame.
  Spawn/extra lock events, lost drops and suppressed melt debris do not advance it.
- Three placements always trigger departure; cover never blocks or resets the engine.
  Countdown visuals, 0.45 s ignition and 1.15 s ascent use scaled time. No ignition after
  game over or while the Pyramid is being rescued/void-consumed.
- Takeoff disables the original collision body, severs connected welds, removes tracking,
  raises the ordinary destruction-accounting event once, and wakes/revalidates supports.
  Only a renderer copy flies behind the tower. It has no collider, rigidbody, controller,
  targetability or height contribution. Falling neighbours still use normal loss rules.
- The engine is visually subordinate to the Egyptian monument. The first spacecraft-like
  grey/teal chassis was rejected; keep warm carved sandstone and amber sun seals. All
  effects ride the collider-less PieceSkin. The departure borrows `rescue_beam` once;
  charge stages are visual, with no looping ambience or repeated alert sounds.
- `Tools > MadTowers > Bake Cyber Pyramid Art` (play mode) rebuilds the static NEXT sprite
  and Vault poster from the runtime shader, preserving their GUIDs/import settings. The
  old Python pyramid generator is retained only as an explicit legacy-art tool. The
  Vault/debut demo uses isolated physical puppets, three illustrative placements and
  the same visual launch; it never sends events into the live ledger.
