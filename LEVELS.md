...
| Power-ups | choice every **10** blocks · pool: Extra Life, Slow Time, Anchor Brick, Freeze, Extract, Hardline · slowMotionScale **0.5** |
...
brick becomes an Anchor, one brick only), Freeze (Epic), Extract (Epic), Hardline (Epic). Adding more: see the doc
