...
| Imported layer **flipbook** | `spriteBackdropLayers[i].animationFrames` (Sprite[]) + `animationFps` | Endless loop of vendor frame animations — Crimson Core's retro sun (20 scrolling-band frames @ 12 fps, chapter-owned tinted copies). Frames must match the base sprite's size; wall-clock-driven so tiles stay in sync (validator lints misconfig) | `.Elements.cs` |
