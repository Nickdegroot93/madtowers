...
/// <summary>
/// Project-wide impact/destruction juice shared by BOTH abilities and block variants - the "this hit
/// had weight" punch, the every-cell burst, and the standard block-shatter destroy. Plain static
/// methods; callers pass whatever parameters their effect needs. (Lives in Core, not under Abilities,
/// because block variants - Bomb, Maw, Magma, Boulder, Tremor - use these just as much as abilities do.)
///
/// Rules for effects that touch the world (from PHYSICS.md - binding):
/// - NEVER write position/rotation on landed blocks; velocity only (ApplyJolt) or lifecycle
///   (FreezeInPlace, Destroy).
/// - Spawned static geometry must match the world contract (friction 0.95, footprint 0.94, corner
///   radius 0.06) and never materialize intersecting the falling piece.
/// </summary>
