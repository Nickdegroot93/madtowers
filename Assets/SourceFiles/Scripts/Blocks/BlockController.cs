...
public class BlockController : MonoBehaviour
{
    ...
    // Resources folder holding the whole-piece sprites (piece_I.png ... piece_Z.png) for the
    // active theme. Deliberately NOT serialized: Unity bakes serialized defaults into its
    // imported prefab cache, so after a folder rename the block prefabs kept resurrecting the
    // stale path until manually reimported. Theme code sets this at runtime for theme skins.
    public static string SkinResourcesFolder = "Skins/Classic";
    ...
    {
        if (string.IsNullOrEmpty(SkinResourcesFolder)) return;

        string shape = ResolveShapeName();
        if (string.IsNullOrEmpty(shape)) return;

        Sprite pieceSprite = Resources.Load<Sprite>($"{SkinResourcesFolder}/piece_{shape}");
        if (pieceSprite == null)
        {
            Debug.LogWarning(
                $"[BlockSkin] No sprite at Resources/{SkinResourcesFolder}/piece_{shape} " +
    ...
    // "Block_T(Clone)" -> "T"
    private string ResolveShapeName()
    {
        string n = name;
        int parenIndex = n.IndexOf('(');
        if (parenIndex >= 0) n = n.Substring(0, parenIndex);
        n = n.Trim();
        int underscoreIndex = n.LastIndexOf('_');
        return underscoreIndex >= 0 && underscoreIndex < n.Length - 1 ? n.Substring(underscoreIndex + 1) : n;
    }
