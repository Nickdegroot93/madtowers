...
/// read by BlockController - this subclass only adds the look and the "no" cue). Like Vortex, "locked" is a
...
/// the clarity gap BLOCKVARIANTS.md flags for Locked.
/// </summary>
[CreateAssetMenu(fileName = "LockedBlockData", menuName = "Stacking/Blocks/Locked Block Variant")]
public class LockedBlockData : BlockData
{
    // Build the gear/chain overlay as soon as the variant is applied (after the chapter skin), so a falling
    // Locked brick already wears it over the chapter colour. Get-or-add avoids a duplicate skin.
    public override void OnApplied(BlockController block)
    {
        if (block == null) return;
        if (!block.TryGetComponent(out LockedBlockSkin skin))
            skin = block.gameObject.AddComponent<LockedBlockSkin>();
        skin.Apply();
    }
